...
x = [1.0 if i in good else 0.0 for i in range(actual.shape[0])]
plt.figure(figsize=(12,4))
plt.plot(x, color="blue")
plt.savefig("./models/{}/backtest/good.png" .format(model))
...
x = [1.0 if i in predicted else 0.0 for i in range(actual.shape[0] - 1 + actual.shape[1])]
plt.figure(figsize=(12,4))
plt.plot(x, color="blue")
plt.savefig("./models/{}/backtest/predicted_days.png" .format(model))
